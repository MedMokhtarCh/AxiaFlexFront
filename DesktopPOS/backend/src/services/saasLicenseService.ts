...
export const ALL_TERMINAL_PLAN_CODES = ['BASIC', 'PRO', 'ENTERPRISE'] as const;
const DEFAULT_MODULES_BY_PLAN: Record<string, string[]> = {
  BASIC: ['dashboard', 'pos', 'open-tickets', 'cash'],
  PRO: [
    'dashboard',
    'pos',
    'open-tickets',
  ...
  ENTERPRISE: [...ALL_SAAS_MODULE_IDS],
};
...
function normalizeTerminalPlans(value: unknown): string[] {
  if (!Array.isArray(value)) return [...ALL_TERMINAL_PLAN_CODES];
  const set = new Set(ALL_TERMINAL_PLAN_CODES as readonly string[]);
  const out = value
    .map((x) => String(x || '').trim().toUpperCase())
    .filter((x) => set.has(x as any));
  return out.length > 0 ? out : [...ALL_TERMINAL_PLAN_CODES];
}

function normalizePlanCode(value: unknown): string {
  const raw = String(value || '').trim().toUpperCase();
  return (ALL_TERMINAL_PLAN_CODES as readonly string[]).includes(raw) ? raw : 'PRO';
}

function normalizeModulesByPlan(value: unknown): Record<string, string[]> {
  const src =
    value && typeof value === 'object' && !Array.isArray(value)
      ? (value as Record<string, unknown>)
      : {};
  const out: Record<string, string[]> = {};
  for (const plan of ALL_TERMINAL_PLAN_CODES) {
    const candidate = src[plan] ?? src[String(plan).toLowerCase()];
    const normalized = normalizeEnabledModules(candidate ?? DEFAULT_MODULES_BY_PLAN[plan]);
    // Enterprise doit toujours avoir tous les modules connus.
    out[plan] = plan === "ENTERPRISE" ? [...ALL_SAAS_MODULE_IDS] : normalized;
  }
  return out;
}
...
export async function ensureLicenseRow(): Promise<SaasTenantLicense> {
  ...
  const created = repo.create({
    ...
    maxTerminals: null,
    enabledModules: null,
    allowedTerminalPlans: null,
    activePlanCode: 'PRO',
    modulesByPlan: DEFAULT_MODULES_BY_PLAN,
...
export async function assertTerminalQuota(nextNewTerminals = 1) {
  await assertLicenseActiveForWrites();
  const lic = await ensureLicenseRow();
  if (lic.maxTerminals == null) return;
  const repo = AppDataSource.getRepository(TerminalNode);
  const n = await repo.count();
  if (n + Math.max(0, nextNewTerminals) > lic.maxTerminals) {
    throw new Error(
      `Limite terminaux atteinte (${lic.maxTerminals}). Augmentez le quota dans le Super Admin.`,
    );
  }
}

export async function isTerminalPlanAllowed(planRaw: string | null | undefined) {
  const lic = await ensureLicenseRow();
  const allowed =
    lic.allowedTerminalPlans == null
      ? [...ALL_TERMINAL_PLAN_CODES]
      : normalizeTerminalPlans(lic.allowedTerminalPlans);
  const plan = String(planRaw || 'BASIC').trim().toUpperCase();
...
export async function getTenantLicenseSnapshot(): Promise<SaasTenantLicensePayload> {
  const lic = await ensureLicenseRow();
  const [users, products, orders, terminals] = await Promise.all([
    AppDataSource.getRepository(User).count(),
    AppDataSource.getRepository(Product).count(),
    AppDataSource.getRepository(Order).count(),
    AppDataSource.getRepository(TerminalNode).count(),
  ]);
  const enabledModules =
    lic.enabledModules == null ? [...ALL_SAAS_MODULE_IDS] : normalizeEnabledModules(lic.enabledModules);
  const activePlanCode = normalizePlanCode(lic.activePlanCode || 'PRO');
  const modulesByPlan = normalizeModulesByPlan(lic.modulesByPlan);
  const planModules = new Set(modulesByPlan[activePlanCode] || []);
  const effectiveModules = enabledModules.filter((m) => planModules.has(m));
  const allowedTerminalPlans =
    lic.allowedTerminalPlans == null
      ? [...ALL_TERMINAL_PLAN_CODES]
      : normalizeTerminalPlans(lic.allowedTerminalPlans);
  return {
    ...
    maxTerminals: lic.maxTerminals,
    usage: { users, products, orders, terminals },
    enabledModules: effectiveModules,
    allowedTerminalPlans,
    activePlanCode,
    modulesByPlan,
...
export async function updateLicenseFromSuperAdmin(dto: SaasLicenseAdminDto): Promise<SaasTenantLicense> {
  ...
  if (dto.maxTerminals !== undefined) lic.maxTerminals = dto.maxTerminals;
  if (dto.enabledModules !== undefined) lic.enabledModules = normalizeEnabledModules(dto.enabledModules);
  if (dto.allowedTerminalPlans !== undefined) {
    lic.allowedTerminalPlans = normalizeTerminalPlans(dto.allowedTerminalPlans);
  }
  if (dto.activePlanCode !== undefined) {
    lic.activePlanCode = normalizePlanCode(dto.activePlanCode);
  }
  if (dto.modulesByPlan !== undefined) {
    lic.modulesByPlan = normalizeModulesByPlan(dto.modulesByPlan);
  }
