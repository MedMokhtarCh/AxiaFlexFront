...
import { maybeFiscalizeTicket } from './nacefFiscalizationService.js';
...
export async function getOrders(params?: { terminalId?: string | null }) {
  ...
  const orders = await repo.find(base);
  ...
  for (const t of tickets as any[]) {
    const oid = String((t as any)?.order?.id || '');
    if (!oid || latestByOrder.has(oid)) continue;
    latestByOrder.set(oid, t);
  }

  return orders.map((o: any) => {
    const t = latestByOrder.get(String(o.id || ''));
    if (!t) return o;
    return {
      ...o,
      fiscalImdf: (t as any)?.fiscalImdf ?? null,
      fiscalStatus: (t as any)?.fiscalStatus ?? null,
      fiscalMode: (t as any)?.fiscalMode ?? null,
      fiscalErrorCode: (t as any)?.fiscalErrorCode ?? null,
    };
  });
}
...
export async function createOrder(body: any) {
      ...
      const row = itemsRepo.create({
        ...
        variantId:
          raw.variantId != null && String(raw.variantId).trim() !== ''
            ...
            : null,
        stockBatchNo:
          raw.stockBatchNo != null && String(raw.stockBatchNo).trim() !== ''
            ? String(raw.stockBatchNo).trim()
            : null,
  ...
  if (shouldPrint) {
    // Ne pas bloquer la caisse: impression prioritaire en arrière-plan.
    void (async () => {
      try {
        await printOrderItemsByPrinter(saved);
      } catch {}
      try {
...
export async function updateOrder(id: string, update: any) {
  ...
  if (Array.isArray(incomingItems)) {
    ...
    for (const raw of incomingItems as any[]) {
      ...
      if (found) {
        ...
        (found as any).variantId =
          raw.variantId !== undefined
            ? raw.variantId != null && String(raw.variantId).trim() !== ''
              ? String(raw.variantId).trim()
              : null
            : (found as any).variantId || null;
        (found as any).stockBatchNo =
          raw.stockBatchNo !== undefined
            ? raw.stockBatchNo != null && String(raw.stockBatchNo).trim() !== ''
              ? String(raw.stockBatchNo).trim()
              : null
            : (found as any).stockBatchNo || null;
  ...
  if (shouldPrint && full) {
    // Priorité au dispatch impression, sans bloquer l'API.
    void (async () => {
      try {
        if (kitchenPrintDelta.length > 0) {
  ...
  if (!opts?.skipTicket) {
    ...
    try {
      ...
      if (reqItems.length > 0) {
        ...
        // Fallback full ticket lines when no partial-item breakdown is provided.
        const allOrderItems = await oiRepo.find({ where: { order: { id } } as any });
        for (const ordItem of allOrderItems as any[]) {
          const q = Number((ordItem as any).quantity || 0);
          if (q <= 0) continue;
          const up = Number((ordItem as any).unitPrice || 0);
          const ti = tRepo.manager.getRepository(TicketItem).create({
            ticket: ticket as any,
            orderItemId: (ordItem as any).id,
            productId: (ordItem as any).productId,
            name: (ordItem as any).name,
            unitPrice: up,
            quantity: q,
            total: q * up,
          } as any);
      ...
      await maybeFiscalizeTicket(ticket as any);
      await tRepo.save(ticket as any);
      ...
      const hardMode =
        ...
        String((settings as any)?.nacefEnforcementMode || 'SOFT')
          .trim()
          .toUpperCase() === 'HARD';
      if (hardMode) {
        throw err;
      }
      // Soft mode: ne pas bloquer le paiement si la fiscalisation échoue.
    }
  }
