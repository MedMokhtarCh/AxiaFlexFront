...
const PurchaseManagement: React.FC = () => {
  ...
  const [receiptTourStep, setReceiptTourStep] = useState(0);
  ...
  const RECEIPT_STEPS = [
    "Selectionner le fournisseur et la reference.",
    "Saisir les lignes (produit, quantite, cout).",
    "Verifier le recapitulatif puis enregistrer.",
  ];

  const isReceiptStepActive = (stepIndex: number) =>
    Boolean(showReceiptHelp && receiptTourStep === stepIndex);

  const receiptGlowClass = (stepIndex: number) =>
    isReceiptStepActive(stepIndex)
  ...
  const renderReceiptTab = () => (
    <div className="relative space-y-4">
      ...
      <div className="rounded-2xl border border-indigo-100 bg-indigo-50/60 px-4 py-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() => setShowReceiptHelp((v) => !v)}
        ...
        >
          <HelpCircle size={14} />
          {showReceiptHelp ? "Masquer l'aide" : "Aide guidée"}
        </button>
      </div>
      {showReceiptHelp && (
        <div className="rounded-2xl border border-indigo-200 bg-white shadow-sm p-4 space-y-3 animate-in fade-in duration-200">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-black text-slate-800">Guide de creation - Reception fournisseur</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {RECEIPT_STEPS.map((step, index) => (
              <button
                key={step}
                type="button"
                onClick={() => setReceiptTourStep(index)}
                className={`text-left rounded-xl border px-3 py-2 transition ${
                  isReceiptStepActive(index)
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 flex items-center gap-2">
            ...
            <p className="text-[11px] font-black text-amber-700">
              Etape actuelle: {receiptTourStep + 1}/{RECEIPT_STEPS.length}
            </p>
          </div>
        </div>
      )}

      <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 p-2 transition ${receiptGlowClass(0)}`}>
      ...
      <div className={`space-y-2 p-2 transition ${receiptGlowClass(1)}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
        ...
        <div
          className={`space-y-3 bg-slate-900 text-slate-50 rounded-3xl p-4 flex flex-col justify-between transition ${
            isReceiptStepActive(2)
              ? "ring-4 ring-amber-200 border border-amber-300"
              : ""
          }`}
        >
          <div>
            ...
            {isReceiptStepActive(2) && (
              <p className="mt-2 text-xs font-black text-amber-200 inline-flex items-center gap-1">
                <Lightbulb size={12} className="animate-pulse" />
                Verifie le total avant validation
              </p>
            )}
          </div>
          <button
