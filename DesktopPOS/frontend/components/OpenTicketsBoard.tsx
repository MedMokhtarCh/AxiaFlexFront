        ...
        {sorted.length === 0 ? (
          ...
          <div className="flex gap-3 sm:gap-5 px-1 h-full items-stretch pb-2">
            {sorted.map((order) => {
              ...
              const fiscalStatus = String((order as any).fiscalStatus || "")
                .trim()
                .toUpperCase();
              const fiscalMode = String((order as any).fiscalMode || "")
                .trim()
                .toUpperCase();
              const fiscalError = String((order as any).fiscalErrorCode || "").trim();
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {fiscalStatus === "SIGNED" ? (
                        <span className="text-[9px] font-black uppercase text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md">
                          NACEF SIGNED{fiscalMode ? ` ${fiscalMode}` : ""}
                        </span>
                      ) : fiscalStatus === "REJECTED" ? (
                        <span
                          title={fiscalError || "Fiscalisation rejetée"}
                          className="text-[9px] font-black uppercase text-rose-700 bg-rose-100 px-2 py-0.5 rounded-md"
                        >
                          NACEF REJECTED
                        </span>
                      ) : null}
