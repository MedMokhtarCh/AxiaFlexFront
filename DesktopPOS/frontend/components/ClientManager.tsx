...
const CLIENT_UI_PREFS_KEY = "client_manager_ui_prefs_v1";
const CONFIRM_TEXT_DELETE = "Supprimer";
const CONFIRM_TEXT_CANCEL = "Annuler";
...
const isOrderPaid = (order: Order) => {
  ...
  const paymentsTotal = Array.isArray(order.payments)
    ? order.payments.reduce(
        (sum, payment) => sum + Number(payment?.totalPaid || 0),
        0,
      )
    : 0;
...
const ClientManager: React.FC = () => {
  ...
  const [clientTypeFilter, setClientTypeFilter] = useState<
    "ALL" | "PERSON" | "COMPANY"
  >("ALL");
  const [clientMinAmount, setClientMinAmount] = useState("");
  const [clientMaxAmount, setClientMaxAmount] = useState("");
  const [clientLastInvoiceFrom, setClientLastInvoiceFrom] = useState("");
  const [clientLastInvoiceTo, setClientLastInvoiceTo] = useState("");
  const [clientPage, setClientPage] = useState(1);
  const [clientSortBy, setClientSortBy] = useState<
    "name" | "type" | "code" | "invoices" | "amount"
  >("name");
  ...
  const goToTourStep = (nextStep: number) => {
    ...
    setTourStep(next);
    if (next === 0) setWorkspaceView("create");
    if (next === 1) setWorkspaceView("tickets");
    if (next === 2) setWorkspaceView("invoices");
  };

  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(CLIENT_UI_PREFS_KEY);
      if (!raw) return;
      const prefs = JSON.parse(raw) as any;
      if (typeof prefs.clientQuery === "string") setClientQuery(prefs.clientQuery);
      if (
        prefs.clientTypeFilter === "ALL" ||
        prefs.clientTypeFilter === "PERSON" ||
        prefs.clientTypeFilter === "COMPANY"
      ) {
        setClientTypeFilter(prefs.clientTypeFilter);
      }
      if (typeof prefs.clientMinAmount === "string")
        setClientMinAmount(prefs.clientMinAmount);
      if (typeof prefs.clientMaxAmount === "string")
        setClientMaxAmount(prefs.clientMaxAmount);
      if (typeof prefs.clientLastInvoiceFrom === "string")
        setClientLastInvoiceFrom(prefs.clientLastInvoiceFrom);
      if (typeof prefs.clientLastInvoiceTo === "string")
        setClientLastInvoiceTo(prefs.clientLastInvoiceTo);
      if (typeof prefs.fromDate === "string") setFromDate(prefs.fromDate);
      if (typeof prefs.toDate === "string") setToDate(prefs.toDate);
      if (typeof prefs.serverFilter === "string") setServerFilter(prefs.serverFilter);
      if (typeof prefs.historyFromDate === "string")
        setHistoryFromDate(prefs.historyFromDate);
      if (typeof prefs.historyToDate === "string") setHistoryToDate(prefs.historyToDate);
      if (typeof prefs.historyServerFilter === "string")
        setHistoryServerFilter(prefs.historyServerFilter);
      if (
        prefs.historyPaymentFilter === "ALL" ||
        prefs.historyPaymentFilter === "PAID" ||
        prefs.historyPaymentFilter === "UNPAID"
      ) {
        setHistoryPaymentFilter(prefs.historyPaymentFilter);
      }
      if (
        prefs.workspaceView === "create" ||
        prefs.workspaceView === "tickets" ||
        prefs.workspaceView === "invoices"
      ) {
        setWorkspaceView(prefs.workspaceView);
      }
      if (
        prefs.clientSortBy === "name" ||
        prefs.clientSortBy === "type" ||
        prefs.clientSortBy === "code" ||
        prefs.clientSortBy === "invoices" ||
        prefs.clientSortBy === "amount"
      ) {
        setClientSortBy(prefs.clientSortBy);
      }
      if (prefs.clientSortDir === "asc" || prefs.clientSortDir === "desc") {
        setClientSortDir(prefs.clientSortDir);
      }
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(CLIENT_UI_PREFS_KEY, JSON.stringify(payload));
    } catch {
  ...
  const clientInvoiceTotalById = useMemo(() => {
    const map = new Map<string, number>();
    for (const inv of invoices) {
      const key = String(inv.clientId || "");
      if (!key) continue;
      map.set(key, Number(map.get(key) || 0) + Number(inv.total || 0));
    }
    return map;
  }, [invoices]);
  const clientInvoicesCountById = useMemo(() => {
    const map = new Map<string, number>();
    for (const inv of invoices) {
      const key = String(inv.clientId || "");
      if (!key) continue;
  ...
  const clientTicketsCountById = useMemo(() => {
    const map = new Map<string, number>();
  ...
  const filteredClientsList = useMemo(() => {
    ...
    const minAmount = Number(clientMinAmount || 0);
    const maxAmount = Number(clientMaxAmount || 0);
    const from = toDayStart(clientLastInvoiceFrom);
    const to = toDayEnd(clientLastInvoiceTo);
    return clients.filter((c) => {
      if (clientTypeFilter !== "ALL" && String(c.type || "") !== clientTypeFilter)
        return false;
      if (!q) return true;
      const hay = [
        c.name,
      ...
      const amount = Number(clientInvoiceTotalById.get(c.id) || 0);
      if (clientMinAmount && amount < minAmount) return false;
      if (clientMaxAmount && amount > maxAmount) return false;
      if (!clientLastInvoiceFrom && !clientLastInvoiceTo) return true;
      const latest = invoices
        .filter((inv) => inv.clientId === c.id)
        .reduce(
          (max, inv) => Math.max(max, Number(inv.createdAt || 0)),
          0,
        );
      if (!latest) return false;
      if (from !== null && latest < from) return false;
      if (to !== null && latest > to) return false;
      return true;
    });
  }, [
    clients,
  ...
  const sortedClients = useMemo(() => {
    const rows = [...filteredClientsList];
    rows.sort((a, b) => {
      ...
      } else if (clientSortBy === "type") {
        av = String(a.type || "").toLowerCase();
        bv = String(b.type || "").toLowerCase();
      } else if (clientSortBy === "code") {
        av = String(a.code || a.phone || a.email || "").toLowerCase();
        bv = String(b.code || b.phone || b.email || "").toLowerCase();
      } else if (clientSortBy === "invoices") {
        av = Number(clientInvoicesCountById.get(a.id) || 0);
        bv = Number(clientInvoicesCountById.get(b.id) || 0);
      } else if (clientSortBy === "amount") {
        av = Number(clientInvoiceTotalById.get(a.id) || 0);
        bv = Number(clientInvoiceTotalById.get(b.id) || 0);
      }
    ...
    clientSortBy,
    clientSortDir,
    clientInvoicesCountById,
    clientInvoiceTotalById,
  ]);
  const pagedClients = useMemo(() => {
    const start = (clientPage - 1) * clientPageSize;
    return sortedClients.slice(start, start + clientPageSize);
  ...
  const personClientsCount = useMemo(
    () => clients.filter((c) => String(c.type || "") === "PERSON").length,
    [clients],
  );
  const companyClientsCount = useMemo(
    () => clients.filter((c) => String(c.type || "") === "COMPANY").length,
    [clients],
  );
  const unassignedPaidTicketsCount = useMemo(
    () =>
      orders.filter(
        (o) =>
          !o.clientId &&
  ...
  const resetUiFilters = async () => {
    const confirmed = await askConfirm({
      title: "Réinitialiser les filtres",
      message:
        "Tous les filtres et tris de la section Clients & Factures seront remis par défaut.",
      confirmText: "Réinitialiser",
      cancelText: CONFIRM_TEXT_CANCEL,
      tone: "danger",
    });
    if (!confirmed) return;
    setClientQuery("");
    ...
    setClientMinAmount("");
    setClientMaxAmount("");
    setClientLastInvoiceFrom("");
    setClientLastInvoiceTo("");
    setServerFilter("");
    setHistoryServerFilter("");
    ...
    setHistoryToDate(toS);
    try {
      window.localStorage.removeItem(CLIENT_UI_PREFS_KEY);
    } catch {
      // Ignore storage errors.
    }
    notifySuccess("Filtres réinitialisés.");
  };
  const createStep =
  ...
  return (
    <div className="touch-client-page flex h-full flex-col gap-4 lg:flex-row lg:gap-6">
      ...
      {detailClient &&
        createPortal(
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
            <div className="w-full max-w-3xl rounded-3xl border border-slate-200 bg-white shadow-2xl max-h-[90vh] overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                <div>
                  <p className="text-lg font-black text-slate-800">{detailClient.name}</p>
                  <p className="text-xs text-slate-500">
                    {detailClient.type === "COMPANY" ? "Société" : "Personne"} •{" "}
                    {detailClient.phone || detailClient.email || detailClient.code || "-"}
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-2 gap-3 border-b border-slate-100 bg-slate-50 p-4 md:grid-cols-4">
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                  ...
                  <p className="text-base font-black text-slate-800">
                    {clientTicketsCountById.get(detailClient.id) || 0}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                  <p className="text-[10px] font-black uppercase text-slate-400">Factures</p>
                  <p className="text-base font-black text-indigo-700">
                    {clientInvoicesCountById.get(detailClient.id) || 0}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 md:col-span-2">
                  <p className="text-[10px] font-black uppercase text-slate-400">Montant facturé</p>
                  <p className="text-base font-black text-emerald-700">
                    {formatAmount(clientInvoiceTotalById.get(detailClient.id) || 0)}{" "}
                    {settings.currency}
                  </p>
                </div>
              </div>
              <div className="max-h-[52vh] overflow-auto p-4">
                ...
                <div className="space-y-2">
                  {invoices
                    .filter((inv) => inv.clientId === detailClient.id)
                    .slice(0, 20)
                    .map((inv) => (
                      <div
                        key={inv.id}
      ...
      <div className="flex w-full max-h-[40vh] flex-col gap-3 overflow-y-auto rounded-3xl border border-slate-100 bg-white p-4 shadow-sm lg:max-h-none lg:w-80 lg:shrink-0">
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            className={`rounded-xl border px-2 py-2 text-[10px] font-black uppercase ${
          ...
          <button
            type="button"
            onClick={() => setClientTypeFilter("PERSON")}
            className={`rounded-xl border px-2 py-2 text-[10px] font-black uppercase ${
        ...
        <div className="grid grid-cols-2 gap-2">
          <input
            type="number"
            value={clientMinAmount}
            onChange={(e) => setClientMinAmount(e.target.value)}
            placeholder="Montant min"
            className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold"
          />
          <input
            type="number"
            value={clientMaxAmount}
            onChange={(e) => setClientMaxAmount(e.target.value)}
            placeholder="Montant max"
            className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold"
          />
          <input
            type="date"
            value={clientLastInvoiceFrom}
            onChange={(e) => setClientLastInvoiceFrom(e.target.value)}
            className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold"
          />
          <input
            type="date"
            value={clientLastInvoiceTo}
            onChange={(e) => setClientLastInvoiceTo(e.target.value)}
            className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold"
          />
        </div>
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            onClick={() => {
              const rows = sortedClients.map((c) => ({
                name: c.name || "",
                type: c.type === "COMPANY" ? "Societe" : "Personne",
                code: c.code || "",
                phone: c.phone || "",
                email: c.email || "",
                invoices: Number(clientInvoicesCountById.get(c.id) || 0),
                amount: Number(clientInvoiceTotalById.get(c.id) || 0).toFixed(3),
              }));
              const header = "Nom;Type;Code;Telephone;Email;Factures;Montant\n";
              const body = rows
                .map((r) =>
                  [r.name, r.type, r.code, r.phone, r.email, r.invoices, r.amount]
          ...
          <button
            type="button"
            onClick={() => {
              const htmlRows = sortedClients
                .map((c) => {
                  const invCount = Number(clientInvoicesCountById.get(c.id) || 0);
                  const amount = Number(clientInvoiceTotalById.get(c.id) || 0);
                  return `<tr><td>${escapeHtml(c.name)}</td><td>${escapeHtml(c.type === "COMPANY" ? "Société" : "Personne")}</td><td>${invCount}</td><td>${formatAmount(amount)}</td></tr>`;
                })
                .join("");
              const html = `<!doctype html><html><head><meta charset="utf-8"/><title>Liste clients</title><style>body{font-family:Arial;padding:24px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd5e1;padding:8px;font-size:12px}th{background:#f8fafc}</style></head><body><h2>Liste clients</h2><table><thead><tr><th>Nom</th><th>Type</th><th>Factures</th><th>Montant</th></tr></thead><tbody>${htmlRows}</tbody></table></body></html>`;
              const win = window.open("", "_blank");
              if (!win) return;
              win.document.open();
              win.document.write(html);
        ...
        <button
          type="button"
          onClick={() => void resetUiFilters()}
          className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-100"
        >
        ...
        <div className="-mx-1 max-h-[min(320px,45vh)] overflow-auto pr-1 lg:max-h-[calc(100vh-320px)] rounded-2xl border border-slate-100">
          <table className="min-w-full text-[11px]">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-2 py-2 text-left">
                  <button type="button" onClick={() => { setClientSortBy("name"); setClientSortDir((d) => d === "asc" ? "desc" : "asc"); }} className="font-black text-slate-600">Nom</button>
                </th>
                <th className="px-2 py-2 text-left">
                  <button type="button" onClick={() => { setClientSortBy("type"); setClientSortDir((d) => d === "asc" ? "desc" : "asc"); }} className="font-black text-slate-600">Type</button>
                </th>
                <th className="px-2 py-2 text-left">
                  <button type="button" onClick={() => { setClientSortBy("invoices"); setClientSortDir((d) => d === "asc" ? "desc" : "asc"); }} className="font-black text-slate-600">Factures</button>
                </th>
                <th className="px-2 py-2 text-right">
                  <button type="button" onClick={() => { setClientSortBy("amount"); setClientSortDir((d) => d === "asc" ? "desc" : "asc"); }} className="font-black text-slate-600">Montant</button>
                </th>
                <th className="px-2 py-2 text-right font-black text-slate-600">Actions</th>
              </tr>
            </thead>
            <tbody>
              {pagedClients.map((client) => (
                <tr key={client.id} className={`border-t border-slate-100 ${selectedClient === client.id ? "bg-indigo-50" : "bg-white"}`}>
                  <td className="px-2 py-2">
                    <button type="button" onClick={() => setSelectedClient(client.id)} className="text-left">
                      <p className="font-bold text-slate-800">{client.name}</p>
                      <p className="text-[10px] text-slate-500 truncate max-w-[120px]">
                        {client.code || client.phone || client.email || "-"}
                      </p>
                    </button>
                  </td>
                  <td className="px-2 py-2 text-slate-600">
                    {client.type === "COMPANY" ? "Société" : "Personne"}
                  </td>
                  <td className="px-2 py-2 text-slate-600">
                    {clientInvoicesCountById.get(client.id) || 0}
                  </td>
                  <td className="px-2 py-2 text-right font-bold text-slate-800">
                    {formatAmount(clientInvoiceTotalById.get(client.id) || 0)}
                  </td>
                  <td className="px-2 py-2">
                    <div className="flex justify-end gap-1">
                      <button type="button" onClick={() => setDetailClientId(client.id)} className="rounded-lg border border-slate-200 bg-white p-1.5 text-slate-600"><Eye size={12} /></button>
                      <button type="button" onClick={() => beginEditClient(client)} className="rounded-lg border border-indigo-200 bg-indigo-50 p-1.5 text-indigo-700"><Pencil size={12} /></button>
                      <button
                        type="button"
                        onClick={async () => {
                          const confirmed = await askConfirm({
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
        <div className="grid grid-cols-2 gap-2 border-b border-slate-100 bg-slate-50 p-3 sm:grid-cols-5">
          <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
            <p className="text-[10px] font-black uppercase text-slate-400">Clients</p>
            <p className="text-lg font-black text-slate-800">{clients.length}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
            <p className="text-[10px] font-black uppercase text-slate-400">Personnes</p>
            <p className="text-lg font-black text-indigo-700">{personClientsCount}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
            <p className="text-[10px] font-black uppercase text-slate-400">Sociétés</p>
            <p className="text-lg font-black text-violet-700">{companyClientsCount}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
            <p className="text-[10px] font-black uppercase text-slate-400">Factures</p>
            <p className="text-lg font-black text-emerald-700">{invoices.length}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-3 py-2">
            <p className="text-[10px] font-black uppercase text-slate-400">CA facturé</p>
            <p className="text-lg font-black text-slate-800">
              {formatAmount(totalInvoiced)} {settings.currency}
            </p>
          </div>
        </div>
        <div
          className={`flex flex-wrap gap-2 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-indigo-50/30 p-3 sm:p-4 transition ${tourGlowClass(0)}`}
        >
          <button
            type="button"
            ...
            } ${showFormHelp && tourStep === 0 ? "ring-4 ring-amber-200" : ""}`}
          >
            <FileText size={15} />
            Nouvelle facture
          </button>
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-2xl border px-4 py-2.5 text-xs font-black transition-all ${
              workspaceView === "tickets"
                ? "border-indigo-600 bg-indigo-600 text-white shadow-md"
                : "border-slate-200 bg-white text-slate-600 hover:border-indigo-200"
            } ${showFormHelp && tourStep === 1 ? "ring-4 ring-amber-200" : ""}`}
          >
            <ListChecks size={15} />
            Tickets
          </button>
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-2xl border px-4 py-2.5 text-xs font-black transition-all ${
              workspaceView === "invoices"
                ? "border-indigo-600 bg-indigo-600 text-white shadow-md"
                : "border-slate-200 bg-white text-slate-600 hover:border-indigo-200"
            } ${showFormHelp && tourStep === 2 ? "ring-4 ring-amber-200" : ""}`}
          >
            <ScrollText size={15} />
            Factures
          </button>
          ...
          {showFormHelp && (
            <div className="mt-3 rounded-2xl border border-indigo-200 bg-white shadow-sm p-4 space-y-3 animate-in fade-in duration-200">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {[
                  "Etape 1: choisir ou creer un client.",
                  "Etape 2: selectionner les tickets a facturer.",
                  "Etape 3: generer, modifier ou suivre la facture.",
                ].map((step, index) => (
                  <button
                    key={step}
                    type="button"
        ...
        {workspaceView === "create" && (
          <>
            <div className="grid grid-cols-1 gap-2 border-b border-slate-100 bg-white px-4 py-3 sm:grid-cols-3 sm:px-6">
              <div className={`rounded-xl border px-3 py-2 text-xs font-black ${createStep >= 1 ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-slate-50 text-slate-500"}`}>1. Choisir / créer client</div>
              <div className={`rounded-xl border px-3 py-2 text-xs font-black ${createStep >= 2 ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-slate-50 text-slate-500"}`}>2. Sélectionner tickets</div>
              <div className={`rounded-xl border px-3 py-2 text-xs font-black ${createStep >= 3 ? "border-emerald-500 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-50 text-slate-500"}`}>3. Générer facture</div>
            </div>
            ...
            <div className="flex flex-col gap-3 border-b border-slate-100 bg-white p-4 sm:flex-row sm:items-center sm:justify-between sm:p-6">
              <div>
                ...
                <p className="text-[11px] font-bold text-amber-600 mt-1">
                  Sans client affecté: {unassignedPaidTicketsCount}
                </p>
              </div>
