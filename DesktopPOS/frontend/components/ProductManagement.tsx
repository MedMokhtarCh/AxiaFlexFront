...
const ProductManagement: React.FC<ProductManagementProps> = ({
  ...
  const tvaCatalogOptions = useMemo(() => {
    const raw = Array.isArray((settings as any)?.tvaCatalog)
      ? (settings as any).tvaCatalog
      : [];
    return raw
      .map((entry: any) => ({
      ...
      .filter((entry: any) => entry.code.length > 0);
  }, [settings]);
  const applyTaxCodeWithSuggestedRate = (nextTaxCodeRaw: string) => {
    const nextTaxCode = String(nextTaxCodeRaw || "").trim().toUpperCase();
    setTaxCode(nextTaxCode);
    if (!nextTaxCode) return;
    const selected = tvaCatalogOptions.find((entry) => entry.code === nextTaxCode);
    if (selected && Number.isFinite(selected.rate)) {
      setTaxRate(String(selected.rate));
    }
  };
  ...
  const openEditModal = (p: Product) => {
    ...
    setTaxRate(
      p.taxRate !== undefined && p.taxRate !== null ? String(p.taxRate) : "",
    );
  ...
  const TOUR_STEPS: Array<{
  ...
  }> = [
    {
      ...
      desc: "Commence par le nom. Le code est optionnel (SKU / PLU).",
      target: () => nameInputRef.current,
    },
    {
      title: "Prix et catégorie",
      desc: "Saisis le prix puis choisis la catégorie pour organiser le POS.",
      target: () => priceInputRef.current || categorySelectRef.current,
    },
    {
      title: "Stock",
      desc: "Choisis le mode de stock (Aucun, Simple, FIFO, Série, Lot).",
      target: () => stockSectionRef.current,
    },
    {
      title: "Promo",
      desc: "Configure un prix promo et une période si nécessaire.",
      target: () => promoSectionRef.current,
    },
    {
      title: "Publication POS",
      desc: "Coche Visible dans POS puis valide avec Enregistrer.",
      target: () => visibleCheckboxRef.current,
    },
  ];
  ...
  const validateForm = () => {
    ...
    if (taxRate.trim()) {
      ...
      if (isNaN(tr) || tr < 0) {
        newErrors.taxRate = "Taux TVA invalide";
      }
    }
    if (taxCode.trim()) {
      const normalizedTaxCode = taxCode.trim().toUpperCase();
      const existsInCatalog = tvaCatalogOptions.some(
        (entry) => entry.code === normalizedTaxCode,
      );
      if (!existsInCatalog) {
        newErrors.taxCode = "Code taxe absent du catalogue TVA";
      }
    }
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {(p.taxRate !== null && p.taxRate !== undefined) || p.taxCode ? (
                          <div className="mt-1 flex flex-wrap gap-1">
                            {p.taxRate !== null && p.taxRate !== undefined ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 text-[10px] font-black">
                                TVA {Number(p.taxRate).toFixed(2)}%
                              </span>
                            ) : null}
                            {p.taxCode ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-[10px] font-black">
                                {p.taxCode}
                              </span>
                            ) : null}
                          </div>
          ...
          <div className="touch-management-modal-panel bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col animate-in slide-in-from-bottom duration-300">
            ...
            <div className="flex flex-col md:flex-row gap-6 p-6 overflow-auto">
              ...
              <div className="flex-1 space-y-6">
                ...
                {showFormHelp && (
                  <div className="rounded-2xl border border-indigo-200 bg-white shadow-sm p-4 space-y-4 animate-in fade-in duration-200">
                    <div className="flex items-center gap-2">
                      <Sparkles size={16} className="text-indigo-600 animate-pulse" />
                      <p className="text-sm font-black text-slate-800">
                        Guide rapide: comment créer un article
                      </p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      >
                        <p className="text-[11px] font-black text-slate-700 flex items-center gap-1.5">
                          <Shapes size={13} className="text-indigo-600" />
                          Etape 1 - Identification
                        </p>
                        <p className="text-[11px] text-slate-600 mt-1">
                          Remplis <strong>Nom</strong> (obligatoire), puis
                          <strong> Code</strong> si tu veux un SKU/PLU interne.
                        </p>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <p className="text-[11px] font-black text-slate-700 flex items-center gap-1.5">
                          ...
                          Etape 2 - Prix et catégorie
                        </p>
                        <p className="text-[11px] text-slate-600 mt-1">
                          Saisis le <strong>Prix</strong> (obligatoire), puis
                          associe une <strong>Catégorie</strong> pour organiser le menu.
                        </p>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <p className="text-[11px] text-slate-600 mt-1">
                          Choisis un mode: <strong>Aucun</strong>, <strong>Simple</strong>,
                          <strong> FIFO</strong>, <strong>Série</strong> ou <strong>Lot</strong>.
                        </p>
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <p className="text-[11px] font-black text-slate-700 flex items-center gap-1.5">
                          <BadgePercent size={13} className="text-fuchsia-600" />
                          Etape 4 - Promo et publication
                        </p>
                        <p className="text-[11px] text-slate-600 mt-1">
                          Optionnel: définis un <strong>prix promo</strong>, puis coche
                          <strong> Visible dans POS</strong> pour l afficher en caisse.
                        </p>
                      </button>
                    </div>
                    <div className="rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2">
                      <p className="text-[11px] font-semibold text-indigo-700">
                        Astuce: l aperçu à droite se met à jour en direct pour vérifier
                        le rendu avant enregistrement.
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <button
                        ...
                        disabled={tourStep <= 0}
                        className="px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-black text-slate-600 disabled:opacity-40"
                      >
                        Précédent
                      </button>
                      <button
                ...
                <div className="space-y-3">
                  <div className={`flex gap-3 p-2 transition ${tourGlowClass(0)}`}>
                  ...
                  <div className={`flex gap-3 p-2 transition ${tourGlowClass(1)}`}>
                    <div className="w-40">
                      <label className="block text-sm font-semibold text-slate-700">
                        Prix *
                      </label>
                  ...
                  <div
                    ref={stockSectionRef}
                    className={`space-y-2 p-2 transition ${tourGlowClass(2)}`}
                  >
                    ...
                    <div className="flex flex-wrap gap-3 items-center mt-1">
                      <label
                        className={`inline-flex items-center gap-2 text-sm text-slate-700 px-2 py-1 rounded-lg transition ${
                          isTourStepActive(4)
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-3 py-2 rounded-xl border text-sm outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent ${
                          errors.taxRate ? "border-red-300" : "border-slate-200"
                        }`}
                        placeholder="ex: 19"
                      />
                      {errors.taxRate && (
                    ...
                    <div>
                      ...
                      <select
                        value={taxCode}
                        onChange={(e) => applyTaxCodeWithSuggestedRate(e.target.value)}
                        className={`w-full px-3 py-2 rounded-xl border text-sm outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent ${
                          errors.taxCode ? "border-red-300" : "border-slate-200"
                        }`}
                      >
                        <option value="">Aucun code taxe</option>
                        {tvaCatalogOptions.map((entry) => (
                          <option key={entry.code} value={entry.code}>
                            {entry.code}
                            {Number.isFinite(entry.rate) ? ` (${entry.rate.toFixed(2)}%)` : ""}
                ...
                <div
                  ref={promoSectionRef}
                  className={`space-y-3 p-2 transition ${tourGlowClass(3)}`}
                >
                  <h4 className="text-sm font-semibold text-slate-700">
                    Prix promotionnel
                    {isTourStepActive(3) && (
