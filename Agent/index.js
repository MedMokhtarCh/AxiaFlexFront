...
const POLL_MS = Math.max(1000, Number(process.env.AGENT_POLL_MS || 3000));
...
async function ensureRegistered() {
  ...
  const reg = await cloudFetch("/pos/agent/register", {
    ...
    body: JSON.stringify({
      ...
      agentVersion: "0.2.0",
      capabilities: { rawTextPrint: true, htmlPrint: true, windows: true },
    }),
  });
...
async function printHtmlBase64(printerName, htmlBase64) {
  const tmpHtml = path.join(os.tmpdir(), `axiaflex-print-${Date.now()}.html`);
  const tmpPdf = path.join(os.tmpdir(), `axiaflex-print-${Date.now()}.pdf`);
...
async function resolvePrinterNameForJob(job, payload) {
  const targetLocalId = String(job?.targetPrinterLocalId || payload?.printerLocalId || "").trim();
  const targetName = String(job?.targetPrinterName || payload?.printerName || "").trim();
